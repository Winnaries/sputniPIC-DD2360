    ...
    for (register int i = 0; i < grd->nxc; i++)
        for (register int j = 0; j < grd->nyc; j++)
            for (register int k = 0; k < grd->nzc; k++){
                // X - component
                vecFieldCx[i][j][k] = .125 * (vecFieldNx[i][j][k] + vecFieldNx[i + 1][j][k] + vecFieldNx[i][j + 1][k] + vecFieldNx[i][j][k + 1] + vecFieldNx[i + 1][j + 1][k] + vecFieldNx[i + 1][j][k + 1] + vecFieldNx[i][j + 1][k + 1] + vecFieldNx[i + 1][j + 1][k + 1]);
                // Y - component
                vecFieldCy[i][j][k] = .125 * (vecFieldNy[i][j][k] + vecFieldNy[i + 1][j][k] + vecFieldNy[i][j + 1][k] + vecFieldNy[i][j][k + 1] + vecFieldNy[i + 1][j + 1][k] + vecFieldNy[i + 1][j][k + 1] + vecFieldNy[i][j + 1][k + 1] + vecFieldNy[i + 1][j + 1][k + 1]);
                // Z - component
                vecFieldCz[i][j][k] = .125 * (vecFieldNz[i][j][k] + vecFieldNz[i + 1][j][k] + vecFieldNz[i][j + 1][k] + vecFieldNz[i][j][k + 1] + vecFieldNz[i + 1][j + 1][k] + vecFieldNz[i + 1][j][k + 1] + vecFieldNz[i][j + 1][k + 1] + vecFieldNz[i + 1][j + 1][k + 1]);
            }
